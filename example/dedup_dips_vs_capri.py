"""DIPS �?CAPRI 同源：用 MMseqs2 �?CAPRI 参考序列比对到 DIPS 复合物序列，
identity �?阈值的 DIPS 复合物写�?exclude 列表，供 prep_dips.py --exclude_file 使用�?

前置�?
  1. 已跑�?prep_dips.py 的下�?拆链阶段（split_pdbs/<stem>_receptor.pdb / _ligand.pdb 就位�?
     —�?即使表面没生成也行，本脚本只读拆�?PDB 提序列�?
  2. 服务器已�?mmseqs2�? conda install -c bioconda mmseqs2   �?  apt 自带二进�?
  3. capri_ref.fasta 已由 extract_capri_sequences.py 生成�?

流程�?
  A. 遍历 split_dir 下所�?<stem>_receptor.pdb / _ligand.pdb，提序列 -> dips_seqs.fasta
     （header = <stem>__R / <stem>__L，便于回溯到复合�?stem�?
  B. mmseqs easy-search  capri_ref.fasta  dips_seqs.fasta  ->  命中�?
  C. 命中(identity≥id_thresh �?coverage≥cov_thresh)�?stem 收集�?set
  D. 把这�?stem 对应�?PDB 4字符前缀 + stem 本身都写�?exclude
     —�?prep_dips.py �?read_exclude �?4 字符前缀匹配，这里额外输�?stem 级精确列表�?
...
def build_dips_fasta(split_dir, out_fasta, min_len=20):
    """遍历 split_dir �?*_receptor.pdb/_ligand.pdb，写 DIPS 序列 FASTA�?
...
def run_mmseqs(capri_fasta, dips_fasta, work_dir, id_thresh, cov_thresh, threads):
    """mmseqs easy-search: query=CAPRI, target=DIPS。返回命�?tsv 路径�?""
    os.makedirs(work_dir, exist_ok=True)
    res_tsv = os.path.join(work_dir, 'capri_vs_dips.tsv')
    tmp = os.path.join(work_dir, 'mmseqs_tmp')
    mmseqs = shutil.which('mmseqs')
    if not mmseqs:
        sys.exit('�?未找�?mmseqs，请�?conda install -c bioconda mmseqs2')
    # --min-seq-id �?search 阶段预过滤；-c/--cov-mode 控制覆盖�?
    cmd = [
...
def collect_hits(res_tsv, id_thresh, cov_thresh):
    """�?mmseqs 结果收集命中�?DIPS stem。target header = <stem>__R/L�?""
    stems = set()
...
def main():
    ...
    ap.add_argument('--out_exclude', required=True, help='输出 exclude 列表(�?prep_dips.py)')
    ap.add_argument('--out_exclude_stems', default=None,
                    help='额外输出 stem 级精确命中列�?可�?')
    ap.add_argument('--id_thresh', type=float, default=0.30,
                    help='序列 identity 阈值，�?则判同源排除（发表常�?0.30�?)
    ...
    if args.reuse_fasta and os.path.exists(dips_fasta):
        ...
        print('── A. �?DIPS 拆链 PDB 提序�?──')
        n = build_dips_fasta(args.split_dir, dips_fasta)
        print(f'  写出 {n} �?DIPS 链序�?-> {dips_fasta}')
    ...
    print(f'  命中（同源需排除）的 DIPS 复合�? {len(stems)} �?)

    # exclude 列表：prep_dips.py read_exclude �?4 字符前缀匹配�?
    # stem 形如 <pdbid>_<rec>_<lig>，取�?4 字符�?PDB ID 前缀�?
    prefixes = sorted({s[:4] for s in stems})
    with open(args.out_exclude, 'w') as f:
        f.write('# �?dedup_dips_vs_capri.py 生成：与 CAPRI Score_set 序列同源�?DIPS PDB 前缀\n')
        f.write(f'# id_thresh={args.id_thresh} cov_thresh={args.cov_thresh} '
                f'命中复合�?{len(stems)} 唯一前缀={len(prefixes)}\n')
        for p in prefixes:
            f.write(p + '\n')
    print(f'── D. exclude 列表 -> {args.out_exclude}（{len(prefixes)} �?PDB 前缀�?)

    if args.out_exclude_stems:
        with open(args.out_exclude_stems, 'w') as f:
            for s in sorted(stems):
                f.write(s + '\n')
        print(f'  stem 级精确列�?-> {args.out_exclude_stems}')
