"""
Score Shirali BM5_15complexes (DeepRank-GNN hold-out) with Tscore.
...
Usage:
  # E0 baseline (pretrain + mdn)
  python example/eval_bm5_15_tscore.py \\
      --data_dir /path/to/BM5_15complexes \\
      --labels_csv /path/to/BM5_scores&labels.csv \\
      --checkpoint Trained_models/pretrain_with_sasa/Tscore_best.chk \\
      --score_type mdn \\
      --out results/bm5_15_tscore_e0.csv \\
      --n_workers 16

  # E1 after pairwise fine-tune (use energy)
  python example/eval_bm5_15_tscore.py \\
      --checkpoint Trained_models/finetune_pairwise_rank/Tscore_best.chk \\
      --score_type energy \\
      --out results/bm5_15_tscore_e1.csv \\
      ...
"""
...
from tscore.models import DeepDock_PPI, NO_INTERFACE_SCORE, ppi_score
...
def success_at_k(rows, k, higher_better=True, label_key='label', score_key='tscore'):
...
def summarize_target(pid, rows):
    ...
    scores = [r['tscore'] for r in labeled if np.isfinite(r['tscore'])]
    labels = [r['label'] for r in labeled if np.isfinite(r['tscore'])]
    out = {
        'pid': pid,
        'n_models': len(rows),
...
def main():
    ...
    work_root = args.workdir or tempfile.mkdtemp(prefix='bm5_15_tscore_')
    ...
    detail_fields = [
        'pid', 'pdb', 'label', 'capri_quality', 'tscore', 'mdn', 'energy',
