"""
Evaluate Tscore on the official DB5 apo/holo data from:
...
candidate poses with Tscore and computes the paper-aligned metrics:
...
Score type (E0/E1):
  --score_type mdn      # E0: pretrained MDN baseline
  --score_type energy   # E1: after pairwise DockQ fine-tune (recommended)
...
from tscore.models import DeepDock_PPI, ppi_score
from tscore.prepare_target.cropInterface import crop_ply_file
from tscore.utils.data import prepare_complex
...
DETAIL_FIELDS = [
    ...
    'pose_model', 'input_rank', 'tscore_rank',
...
def write_aggregate(summary_path, pose_models):
    ...
    for k in SUCCESS_KS:
        tscore_key = f'tscore_success@{k}'
        paper_key = f'paper_success@{k}'
        oracle_key = f'oracle_success@{k}'
        tscore_rate = sum(int(fval(r, tscore_key, fval(r, f'success@{k}'))) for r in rows) / max(n_targets, 1)
        ...
        out[f'success@{k}'] = tscore_rate
        out[tscore_key] = tscore_rate
        out[paper_key] = paper_rate
        out[oracle_key] = oracle_rate
        out[f'delta_tscore_minus_paper@{k}'] = tscore_rate - paper_rate
...
def evaluate_target(args, paper_eval, model, dist_threshold, in_channels, device,
    ...
    for k in SUCCESS_KS:
        summary[f'tscore_success@{k}'] = success_at(ranked, k)
        summary[f'paper_success@{k}'] = success_at(paper_ranked, k)
        summary[f'oracle_success@{k}'] = success_at(oracle_ranked, k)
        summary[f'success@{k}'] = summary[f'tscore_success@{k}']
    return detail_rows, summary


def main():
    parser = argparse.ArgumentParser(
        description='Tscore reranking on official DB5 apo/holo paper data'
    )
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix='tscore_db5_paper_'))
            ...
            for i, target in enumerate(targets, 1):
                ...
                if summary.get('status') == 'done':
                    print(
                        ...
                        f"Tscore@1={summary.get('tscore_success@1', 0)} "
    ...
    if agg:
        ...
        print(
            f"Tscore@1={100 * agg_row['tscore_success@1']:.2f}%  "
            f"Paper@1={100 * agg_row['paper_success@1']:.2f}%  "
            f"Tscore@5={100 * agg_row['tscore_success@5']:.2f}%  "
